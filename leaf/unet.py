...
class UNetModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    """
    ...
    def __init__(
        ...
        attention_resolutions: Tuple = (1, 2, 4, 8),
        dropout: float = 0.0,
    ...
    def timestep_embedding(
        ...
        embedding = embedding.to(dtype=sample.dtype)
        return embedding
    ...
    def forward(
        ...
        hs = []
        t_emb = self.timestep_embedding(sample, timestep, self.model_channels, repeat_only=False)
        if t_emb.dim() == 1:
            t_emb = t_emb.repeat(sample.shape[0], 1)
        emb = self.time_embed(t_emb)

        for module in self.input_blocks:
            sample = module(sample, emb, encoder_hidden_states)
            hs.append(sample)
        sample = self.middle_block(sample, emb, encoder_hidden_states)
        for module in self.output_blocks:
            sample = torch.cat([sample, hs.pop()], dim=1)
            sample = module(sample, emb, encoder_hidden_states)
